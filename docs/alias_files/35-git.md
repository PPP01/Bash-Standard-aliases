...
- `gl`: Kompakter Graph-Log über alle Branches
...
| gaa | git add --all | Fügt alle Änderungen zur Git-Staging-Area hinzu. | git add --all |
...
| gca | git commit --amend | Ändert den letzten Commit (amend). | git commit --amend |
| gcan | git commit --amend --no-edit | Ändert den letzten Commit ohne neue Nachricht. | git commit --amend --no-edit |
| gd | git diff | Zeigt nicht gestagte Änderungen. | git diff |
| gds | git diff --staged | Zeigt gestagte Änderungen. | git diff --staged |
...
| gr | git restore | Setzt Änderungen im Working Tree auf HEAD zurück. | git restore <file> |
| grs | git restore --staged | Entfernt Dateien aus der Staging-Area. | git restore --staged <file> |
| grsoft1 | git reset --soft HEAD~1 | Setzt den letzten Commit zurück und behält alle Änderungen im Staging-Bereich. | git reset --soft HEAD~1 |
| grhard1 | git reset --hard HEAD~1 | Verwirft den letzten Commit inklusive Working-Tree-Änderungen unwiderruflich. | git reset --hard HEAD~1 |

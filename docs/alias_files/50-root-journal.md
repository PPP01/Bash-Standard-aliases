...
| logs | Journal pro Service | Zeigt Journal-Logs fuer einen Dienst mit optionaler Zeilenanzahl (root-Funktion). | journalctl -u <service> -n <X> --no-pager (Standard: X=50) |
...
| log_clean | Journal bereinigen | Bereinigt Journal-Logs nach Aufbewahrungszeit/Groesse (root-Funktion). | journalctl --vacuum-time=<tage>d --vacuum-size=<groesse> (Standard: 2d, 100M) |

...
# shellcheck disable=SC1090
source "${_alias_help_impl_dir}/alias_menu_engine.sh"
...
_alias_menu_category() {
  ...
  while true; do
    ...
    if [ "${max_select}" -le 0 ]; then
      selected=0
    elif [ "${selected}" -lt 1 ] || [ "${selected}" -gt "${max_select}" ]; then
      selected=1
    fi
    ...
    case "${choice}" in
      ...
      down)
        if [ "${max_select}" -gt 0 ]; then
          if [ "${selected}" -ge "${max_select}" ]; then
            selected=1
          else
            selected=$((selected + 1))
          fi
...
_alias_menu_all_categories() {
  ...
  while true; do
    ...
    case "${choice}" in
      ...
      down)
        if [ "${#menu_categories[@]}" -gt 0 ]; then
          if [ "${selected}" -ge "${#menu_categories[@]}" ]; then
            selected=1
          else
            selected=$((selected + 1))
          fi
        ...
        ;;
      right|'')
        if [ "${#menu_categories[@]}" -le 0 ] || [ "${selected}" -lt 1 ]; then
          echo "$(_alias_text alias_invalid)"
          return 1
        fi
        category="${menu_categories[$((selected - 1))]}"
...
_alias_pick_category_interactive() {
  ...
  while true; do
    ...
    rendered_entries=0
    echo ""
    printf '%b%s%b\n' "${BASH_ALIAS_HELP_COLOR_MENU_TITLE}" "$(_alias_text categories_title)" "${BASH_ALIAS_HELP_COLOR_RESET}"
    marker=" "
    line_color=""
    printf ' %b%s%b %b%3d) %-12s%b\n' \
      "$(_alias_menu_highlight_marker_color)" "${marker}" "${BASH_ALIAS_HELP_COLOR_RESET}" \
      "${line_color}" 0 "$(_alias_text categories_back)" "${BASH_ALIAS_HELP_COLOR_RESET}"
